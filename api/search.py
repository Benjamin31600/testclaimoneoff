...
class Parser(HTMLParser):
    ...
    def handle_starttag(self,tag,attrs):
        a=dict(attrs)
        if tag=='a' and a.get('href'): self.href=a['href']; self.links.append((a['href'],''))
...
def fetch(url,timeout=12):
    req=urllib.request.Request(url,headers={'User-Agent':'Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) AppleWebKit/605.1.15 Safari/604.1'})
...
def official_urls():
    urls=[]
    for path in ['/sitemap.xml','/sitemap_index.xml','/fr/']:
        try:
            h=fetch(OFFICIAL+path,15)
            urls += re.findall(r'<loc>\s*(https?://[^<]+)\s*</loc>',h,re.I)
            if path=='/fr/':
                p=Parser();p.feed(h);urls += [urljoin(OFFICIAL+'/',u) for u,_ in p.links]
        except Exception: pass
    return list(dict.fromkeys(u for u in urls if '/fr/membres/' in u))

def official_results(city,lat,lon,radius):
    out=[];errors=[]; urls=official_urls()
    if not urls: errors.append('official:no-member-index')
    for u in urls[:250]:
        try:
            h=fetch(u,10);p=Parser();p.feed(h);text=' '.join(p.text)
            status,infant,sub=classify(text)
            if not infant:continue
            m=re.search(r'(\d+)\s+places?\s+0-18 mois',text,re.I); places=int(m.group(1)) if m else 1
            ...
            out.append({'name':name,'city':city2,'address':'Adresse civique non publiée sur la fiche','postalCode':postal,'phone':tm.group(1).strip() if tm else '','email':em.group(1).strip() if em else '','infantPlaces':places,'status':status,'subsidized':True,'distanceKm':dist,'source':u,'sourceEngine':'Guichet unique Laurentides','published':dm.group(1).strip() if dm else None,'snippet':text[:1000],'score':100})
...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        official,err=official_results(city,lat,lon,radius);providers.extend(official);sources.append('Guichet unique Laurentides');errors.extend(err)
